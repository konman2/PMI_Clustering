...
def run(flag='p',file='Graphs/airport_ww/network.pkl',name='airport',times='micro',precomp=None,subgraph_locs=[]):
    ...
    while(total<P.shape[0]):
        ...
        final_output[best_l] = next_clust
        valid = np.sum(valid_locs & best_l,axis=1) == 0
        
        valid_locs = valid_locs[valid]
        vals = vals[valid]
        ...
        #print(predictions[0])
    _,o = np.unique(final_output,return_inverse=True)
    e = time.time()
    runtimes.append(e-s)
    outfile = open(f'./results/results_{name}_strict-({int(times[0])},{int(times[-1])},{len(times)}).txt', 'w')
