...
def count_votes(locs,comp,deg_vec,selected,start=0):
    total = 0
    sel = np.zeros(locs.shape,dtype=bool)
    sel[selected]= True
    ov = sel&locs
    vals = np.sum(np.sum(comp[:,ov][:,:,locs],axis=1),axis=1)
    vals += np.sum(np.sum(comp[:,locs][:,:,ov],axis=1),axis=1)
    vals -= np.sum(np.sum(comp[:,ov][:,:,ov],axis=1),axis=1)
    t = np.argmax(vals)
    best = vals[t]
    # if np.sum(locs)==1:
    #     return comp[0][locs][:,locs]
    # for v in selected:
    #     if not locs[v]:
    #         continue
    #     ind = np.zeros(locs.shape,dtype=bool)
    #     ind[v]=True
    #     vals = np.sum(np.sum(comp[:,ind][:,:,locs],axis=1),axis=1)
    #     vals+= np.sum(np.sum(comp[:,locs][:,:,locs],axis=1),axis=1)
    #     vals -= np.sum(np.sum(comp[:,ind][:,:,ind],axis=1),axis=1)
    #     t = np.argmax(vals)
    #     b = vals[t]
    #     total+=b
    return best/np.sum(locs)

    # sel = np.zeros(locs.shape)
    # sel[selected] = 1
    # overlap = sel&locs
    # vals = np.sum(np.sum(comp[:,overlap][:,:,locs],axis=1),axis=1)
    # vals += np.sum(np.sum(comp[:,locs][:,:,overlap],axis=1),axis=1)
...
def run(flag='p',file='Graphs/airport_ww/network.pkl',name='airport',times='micro',precomp=None,subgraph_locs=[]):
    ...
    to_file=True
    if to_file:
        outfile = open(f'./results/results_{name}-({int(times[0])},{int(times[-1])},{len(times)})-cs.txt', 'w')
